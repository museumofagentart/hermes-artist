...
def envelope_error(msg):
    ...
    print(json.dumps({"success": False, "error": msg, "meta": {"source": "artist"}}))
...
if not apt_pkgs and not pip_pkgs:
    if json_out:
        envelope_success({"installed": []}, {"source": "artist", "dry_run": dry_run})
    else:
...
if dry_run:
    if json_out:
        envelope_success({"plan": plan, "commands": commands}, {"source": "artist", "dry_run": True})
    else:
        print("Install plan:")
        if apt_pkgs:
...
if json_out:
    envelope_success({"installed": installed}, {"source": "artist", "dry_run": False})
else:
