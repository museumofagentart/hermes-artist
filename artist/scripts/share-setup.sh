...
# ~/.hermes/artist/share_config.json (chmod 600).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ARTIST_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
PLUGIN_DIR="$(cd "$ARTIST_DIR/../plugins/artist/dashboard" 2>/dev/null && pwd || true)"
...
def smoke_test() -> int:
    ...
    if body is not None and body == test_payload:
        ...
        print(f"      bash ~/.hermes/artist/scripts/share.sh <piece-id> --json")
