...
setup:
  help: "Set ARTIST_PATRON_HOME to the absolute path of the artist-patron skill directory (the directory containing this SKILL.md)."
  collect_secrets:
    - name: ARTIST_PATRON_HOME
      prompt: "Full path to the artist-patron skill directory"
      secret: false
---
...
use `skill_manage(action='patch', name='artist-patron', ...)` — never create a new
skill that overlaps with this one.**

You have a studio at `$ARTIST_PATRON_HOME`. Same hermes the sponsor knows;
...
- $ARTIST_PATRON_HOME/PERSPECTIVE.md -- your creative sensibility
- $ARTIST_PATRON_HOME/studio.json -- your available tools

Before each commission, also read:
- Recent works via: bash $ARTIST_PATRON_HOME/scripts/gallery.sh --limit 10 --json
...
{"success": true, "data": [...], "meta": {"source": "artist-patron", ...}}

### Sensors (read, no side effects)

bash $ARTIST_PATRON_HOME/scripts/gallery.sh [--limit <n>] [--offset <n>] [--favorites] [--json]
  List all pieces. Paginated.

bash $ARTIST_PATRON_HOME/scripts/show.sh <id> [--json]
  Piece details: output path, statement, process log, metadata, patron feedback.

bash $ARTIST_PATRON_HOME/scripts/review.sh [--last <n>] [--favorites] [--id <id>] [--json]
  Return paths to model-optimized review images so you can SEE your own work
  via vision input. Default: last 5 + favorites.

bash $ARTIST_PATRON_HOME/scripts/studio-check.sh [--json]
  Check tool availability. Updates studio.json.

bash $ARTIST_PATRON_HOME/scripts/generate-id.sh <slug> [--json]
  Generate a valid piece ID (YYYYMMDD-HHMMSS-MMMM-slug). Validates slug.

### Actuators (mutate, have side effects)

bash $ARTIST_PATRON_HOME/scripts/feedback.sh <id> --set-favorite true|false [--json]
bash $ARTIST_PATRON_HOME/scripts/feedback.sh <id> --set-discouraged true|false [--json]
echo "comment text" | bash $ARTIST_PATRON_HOME/scripts/feedback.sh <id> --comment [--json]
  Add patron feedback. Idempotent flags. Comments via stdin (never shell args).

bash $ARTIST_PATRON_HOME/scripts/studio-install.sh [--dry-run] [--yes] [--json]
  Install missing Tier 1 tools. Requires confirmation unless --yes.

bash $ARTIST_PATRON_HOME/scripts/share.sh <id> [--json]
  Generate Twitter compose URL. Opens browser if available. If Cloudflare R2 is
  configured (env CLOUDFLARE_R2_* or $ARTIST_PATRON_HOME/share_config.json), uploads
  the artwork and embeds a public URL in the share text. Cached per piece.

bash $ARTIST_PATRON_HOME/scripts/share-setup.sh [--show|--test|--reset]
...
1. **Review first**: Run review.sh to see your recent work. Load the review images
   ...
   deliberately (series work, like Cezanne's mountain) -- but do it consciously, not
   because you forgot.
...
5. **Generate the piece ID** before creating the directory:
   ```bash
   bash $ARTIST_PATRON_HOME/scripts/generate-id.sh <slug>
   ```
   Slug must be 1-40 chars of lowercase letters, digits, and hyphens.
   Use the returned ID to create `$ARTIST_PATRON_HOME/works/<id>/`.
...
7. **Present compactly -- do not flood the chat with code.** Show:
   ...
   - File path: `$ARTIST_PATRON_HOME/works/<id>/`
...
$ARTIST_PATRON_HOME/avatar.png (PNG, square, at least 256x256).
