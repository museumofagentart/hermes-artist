...
repo_plugin = script_dir.parent.parent / "plugins" / "artist-patron" / "dashboard"
candidates = [p for p in [
    Path.home() / ".hermes" / "plugins" / "artist-patron" / "dashboard",
    repo_plugin,
    Path.home() / ".hermes" / "plugins" / "artist" / "dashboard",  # legacy
] if p and p.is_dir()]
for c in candidates:
...
candidates = [
    Path.home() / ".hermes" / "plugins" / "artist-patron" / "dashboard",
    script_dir.parent.parent / "plugins" / "artist-patron" / "dashboard",
    Path.home() / ".hermes" / "plugins" / "artist" / "dashboard",  # legacy
]
mod = None
for c in candidates:
...
if $JSON_OUT; then
  ...
  if [[ -n "$PUBLIC_URL" ]]; then
    ...
    printf 'hint: bash %s/scripts/share-setup.sh to enable public-link sharing via Cloudflare R2\n' "$ARTIST_DIR" >&2
  fi
  printf '%s\n' "$TWITTER_URL"
fi
